...
describe("buildClaudeCodeModels", () => {
  it("keeps curated models and appends discovered models", () => {
    ...
    expect(
      ...
      { model: "claude-opus-5[1m]", displayName: "Opus 5 (1M)", isDefault: false },
      { model: "claude-opus-4-8[1m]", displayName: "Opus 4.8 (1M)", isDefault: false },
      { model: "claude-opus-4-7[1m]", displayName: "Opus 4.7 (1M)", isDefault: false },
  ...
  it("keeps the curated catalog available when discovery returns no models", () => {
  ...
  it("keeps Opus 5 as the default for older Claude Code releases", () => {
    const result = buildClaudeCodeModels([
      {
        value: "default",
    ...
    expect(result.models.filter((model) => model.isDefault)).toEqual([
      expect.objectContaining({ model: "claude-opus-5[1m]" }),
    ]);
  });
});
