...
import { listRecentWorkspaceFiles, recordWorkspaceFileChanges } from "../src/index.js";
  ...
  it("notices both sides exactly once for one file", () => {
    ...
    try {
      ...
      expect(second).toEqual([
        { filePath: "src/a.ts", threadId: "thread-a", otherThreadId: "thread-b" },
        { filePath: "src/a.ts", threadId: "thread-b", otherThreadId: "thread-a" },
      ]);
      expect(listRecentWorkspaceFiles(db, { hostId: "host", workspacePath: "/repo", now: 1_002 })).toEqual([
        { filePath: "src/a.ts", threadIds: ["thread-a", "thread-b"], observedAt: 1_002 },
      ]);
