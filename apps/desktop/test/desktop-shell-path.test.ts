...
describe("desktop shell PATH loading", () => {
  ...
  it("kills a shell that ignores SIGTERM instead of blocking the app", () => {
    // Interactive zsh ignores SIGTERM; with the default kill signal the probe
    // never ended and the desktop main process hung at launch.
  ...
  it("gives the shell no stdin to wait on", () => {
    // `cat` returns at once on /dev/null; with an open stdin it would block
    // until the timeout.
    const started = Date.now();
    const result = defaultSpawnLoginShellPath({
      command: "/bin/sh",
